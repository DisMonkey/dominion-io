...
export class PastelTheme implements Theme {
  ...
  terrainColor(gm: GameMap, tile: TileRef): Colord {
    ...
    const strategicType = strategicTerrainType(gm, tile);
    if (strategicType === StrategicTerrainType.Forest) {
      return colord({
        r: 104,
        g: 162 - Math.min(mag, 18),
        b: 112,
      });
    }
    if (strategicType === StrategicTerrainType.Desert) {
      return colord({
        r: 214 + Math.min(mag, 18),
        g: 184 + Math.min(mag, 18),
        b: 112,
      });
    }
    if (strategicType === StrategicTerrainType.Snow) {
      return colord({
        r: 205 + Math.min(mag, 20),
        g: 222 + Math.min(mag, 20),
        b: 226 + Math.min(mag, 20),
      });
    }
