...
export class PlayerImpl implements Player {
  ...
  public _units: Unit[] = [];
  private _completedTechs = new Set<Tech>();
  private _activeResearch: ActiveResearch | null = null;
  ...
  completedTechs(): readonly Tech[] {
    return [...this._completedTechs].sort();
  }

  activeResearch(): ActiveResearch | null {
    return this._activeResearch === null ? null : { ...this._activeResearch };
  }

  canResearch(tech: Tech): boolean {
    return (
      canResearchTech(this._completedTechs, this._activeResearch, tech) &&
      this.gold() >= TechDefinitions[tech].cost
    );
  }

  startResearch(tech: Tech, tick: Tick): boolean {
    if (!this.canResearch(tech)) {
      return false;
    }
    const definition = TechDefinitions[tech];
    this.removeGold(definition.cost);
    this._activeResearch = {
      tech,
      startedAt: tick,
      completesAt: tick + definition.durationTicks,
    };
    return true;
  }

  tickResearch(tick: Tick): Tech | null {
    const active = this._activeResearch;
    if (active === null || active.completesAt > tick) {
      return null;
    }
    this._completedTechs.add(active.tech);
  ...
  private canSpawnUnitType(
    ...
    switch (unitType) {
      ...
      case UnitType.Engineer:
        return this.ownedLandUnitSpawn(targetTile);
      case UnitType.Train:
  ...
  ownedLandUnitSpawn(tile: TileRef): TileRef | false {
    return this.mg.isLand(tile) && this.mg.owner(tile) === this ? tile : false;
  }
  ...
  hash(): number {
    return (
      ...
      this.completedTechs().reduce((acc, tech) => acc + simpleHash(tech), 0) +
      (this._activeResearch === null
        ? 0
        : simpleHash(this._activeResearch.tech) +
          this._activeResearch.startedAt +
          this._activeResearch.completesAt)
    );
  }
  toString(): string {
