...
import { FixedSimulationScheduler } from "./runtime/FixedSimulationScheduler";
...
export class ClientGameRunner {
  ...
  private turnScheduler: FixedSimulationScheduler;
  ...
  constructor(
    ...
    this.turnScheduler = new FixedSimulationScheduler({
      fixedStepMs: this.lobby.serverConfig.turnIntervalMs(),
      targetBufferedTurns: 1,
      maxCatchUpTurnsPerFrame: 8,
      onTurn: (turn) => this.worker.sendTurn(turn),
      onFrame: (stats) => {
        this.renderer.setInterpolationAlpha(stats.interpolationAlpha);
        this.renderer.setQueuedSimulationTurns(stats.queuedTurns);
      },
    });
  }
    ...
    const onmessage = (message: ServerMessage) => {
      ...
      if (message.type === "start") {
        ...
        for (const turn of message.turns) {
          this.queueAuthoritativeTurn(turn);
        }
      }
      ...
      if (message.type === "turn") {
        ...
        this.queueAuthoritativeTurn(message.turn);
      }
    };
    this.transport.updateCallback(onconnect, onmessage);
    console.log("sending join game");
    // Ask only for authoritative turns we have not queued yet.
    this.transport.rejoinGame(this.turnsSeen);
  }
  ...
  private queueAuthoritativeTurn(turn: Turn): void {
    ...
    if (turn.turnNumber !== this.turnsSeen) {
      console.warn(
        `turn gap detected: expected ${this.turnsSeen}, received ${turn.turnNumber}`,
      );
    }

    while (turn.turnNumber > this.turnsSeen) {
      this.turnScheduler.enqueue({
