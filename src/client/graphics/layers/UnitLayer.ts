...
export class UnitLayer implements Layer {
  ...
  private readonly nearbyWarshipBuffer: Array<{
    unit: UnitView;
    distSquared: number;
  }> = [];
  private readonly nearbyWarshipSelectionBuffer: UnitView[] = [];
  ...
  private findWarshipsNearCell(clickRef: TileRef): UnitView[] {
    const candidates = this.game.nearbyUnitsInto(
      this.nearbyWarshipBuffer,
      clickRef,
    ...
    const selected = this.nearbyWarshipSelectionBuffer;
    selected.length = 0;
    for (const candidate of candidates) {
