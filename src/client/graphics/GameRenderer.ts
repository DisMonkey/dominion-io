...
export function createRenderer(
  ...
  const layers: Layer[] = [
    ...
    new ParticleLayer(game, eventBus, transformHandler),
    new FogOfWarLayer(game, transformHandler),
...
export class GameRenderer {
  ...
  private queuedSimulationTurns = 0;
  ...
  initialize() {
    ...
    this.isRunning = true;
    this.rafId = requestAnimationFrame((time) => this.renderGame(time));
  ...
  setQueuedSimulationTurns(queuedTurns: number) {
    this.queuedSimulationTurns = Math.max(0, queuedTurns);
  }
  ...
  simulationBacklog(): number {
    return this.queuedSimulationTurns;
  }
