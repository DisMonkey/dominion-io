...
import { decodePacketPayload, encodePacket } from "../core/network/PacketCodec";
import {
  AllPlayersStats,
  ClientHashMessage,
  ClientIntentMessage,
...
function toWebSocketPayload(bytes: Uint8Array): ArrayBuffer {
...
export class Transport {
  ...
  private connectRemote(
    ...
    const wsProtocol =
      configuredWsHost || window.location.protocol === "https:" ? "wss:" : "ws:";
    ...
    this.socket.onmessage = async (event: MessageEvent) => {
      try {
        const parsed = await decodePacketPayload(event.data);
        ...
        if (result.data.type === "ping" && result.data.sentAt !== undefined) {
          this.latencyMs = Math.max(0, performance.now() - result.data.sentAt);
        }
  ...
  public networkLatencyMs(): number | null {
    return this.latencyMs;
  }

  private scheduleReconnect() {
    if (this.isLocal || this.reconnectTimer !== null) {
      return;
    }
    const delayMs = Math.min(3000, 250 * 2 ** this.reconnectAttempt);
  ...
  private sendMsg(msg: ClientMessage) {
    ...
    const packet = toWebSocketPayload(encodePacket(msg));
    if (this.socket.readyState !== WebSocket.OPEN) {
      console.warn("socket not ready, buffering message");
