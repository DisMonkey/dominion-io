//#region node_modules/colorjs.io/dist/color.js
...
function multiplyMatrices(A, B) {
	...
	if (!Array.isArray(B[0])) {
		BM = B.length > 0 ? B.map((x) => [x]) : [[]];
		bVec = true;
	} else BM = B;
	let p = BM[0].length;
	let BM_cols = BM[0].map((_, i) => BM.map((x) => x[i]));
	/** @type {number[] | number[][]} */
	...
	if (m === 1 && aVec) product = product[0];
	if (p === 1 && bVec) if (m === 1 && aVec) return product[0];
	else return product.map((x) => x[0]);
	return product;
}
/**
...
* @returns {Vector3}
*/
function multiply_v3_m3x3(input, matrix, out = [
	0,
	0,
	0
]) {
...
function mapRange(from, to, value) {
	if (!from || !to || from === to || from[0] === to[0] && from[1] === to[1] || isNaN(value) || value === null) return value;
...
function isInstance(arg, constructor) {
	if (arg instanceof constructor) return true;
	const targetName = constructor.name;
	while (arg) {
		const proto = Object.getPrototypeOf(arg);
		const constructorName = proto?.constructor?.name;
		if (constructorName === targetName) return true;
		if (!constructorName || constructorName === "Object") return false;
		arg = proto;
	}
	return false;
}
...
var Type = class {
	...
	*/
	constructor(type, coordMeta) {
		if (typeof type === "object") this.coordMeta = type;
		if (coordMeta) {
		...
		if (typeof type === "string") {
			...
			if (!params) throw new TypeError(`Cannot parse ${type} as a type definition.`);
			this.type = params.groups.type;
			let { min, max } = params.groups;
			if (min || max) this.range = [+min, +max];
		}
	}
	/** @returns {[number, number]} */
	get computedRange() {
		if (this.range) return this.range;
		if (this.type === "<percentage>") return this.percentageRange();
		else if (this.type === "<angle>") return [0, 360];
		return null;
	}
	get unit() {
		if (this.type === "<percentage>") return "%";
		else if (this.type === "<angle>") return "deg";
		return "";
	}
	/**
	...
	*/
	resolve(number) {
		if (this.type === "<angle>") return number;
		let fromRange = this.computedRange;
		let toRange = this.coordRange;
		if (this.type === "<percentage>") toRange ??= this.percentageRange();
	...
	*/
	percentageRange(scale = 1) {
		let range;
		if (this.coordMeta && this.coordMeta.range || this.coordRange && this.coordRange[0] >= 0) range = [0, 1];
		else range = [-1, 1];
		return [range[0] * scale, range[1] * scale];
	}
	static get(type, coordMeta) {
		if (isInstance(type, this)) return type;
...
var Format = class Format {
	...
	*/
	constructor(format, space = format.space) {
		...
		if (!this.coords) this.coords = this.spaceCoords.map((coordMeta) => {
			let ret = ["<number>", "<percentage>"];
			if (coordMeta.type === "angle") ret.push("<angle>");
			return ret;
		});
		this.coords = this.coords.map(
			...
			(types, i) => {
				...
				if (typeof types === "string") types = types.trim().split(/\s*\|\s*/);
	...
	*/
	coerceCoords(coords, types) {
		return Object.entries(this.space.coords).map(([id, coordMeta], i) => {
			let arg = coords[i];
			if (isNone(arg) || isNaN(arg)) return arg;
			...
			if (type.range) types[i] = type.toString();
			return arg;
		});
	}
	/**
	...
	*/
	static get(format, ...args) {
		if (!format || isInstance(format, this)) return format;
		if (format[instance]) return format[instance];
...
function adapt$2(W1, W2, XYZ, options = {}) {
	...
	if (env.M) return multiply_v3_m3x3(env.XYZ, env.M);
...
function parse(str, options) {
	...
	if (env.parsed) {
		...
		if (name === "color") {
			...
			if (!format) {
				...
				if (registryId in ColorSpace.registry) {
					...
					if (cssId) didYouMean = `Did you mean ${str.replace("color(" + id, "color(" + cssId)}?`;
				}
				throw new TypeError(`Cannot parse ${env.str}. ` + (didYouMean ?? "Missing a plugin?"));
			}
			space = format.space;
			if (format.id.startsWith("--") && !id.startsWith("--")) defaults.warn(`${space.name} is a non-standard space and not currently supported in the CSS spec. Use prefixed color(${format.id}) instead of color(${id}).`);
			if (id.startsWith("--") && !format.id.startsWith("--")) defaults.warn(`${space.name} is a standard space and supported in the CSS spec. Use color(${format.id}) instead of prefixed color(${id}).`);
		} else {
			format = ColorSpace.findFormat({
				name,
				type: "function"
		...
		if (env.parsed.lastAlpha) {
			...
			if (meta) meta.alphaType = types.pop();
		}
		let coordCount = format.coords.length;
		if (coords.length !== coordCount) throw new TypeError(`Expected ${coordCount} coordinates for ${space.id} in ${env.str}), got ${coords.length}`);
	...
	} else spaceloop: for (let space of ColorSpace.all) for (let formatId in space.formats) {
	...
	if (!ret) throw new TypeError(`Could not parse ${str} as a color. Missing a plugin?`);
...
function parseFunction(str) {
	...
	if (parts) {
		...
		let separators = parts[2].replace(regex.singleArgument, ($0, rawArg) => {
			...
			if ($0.startsWith("/") || name !== "color" && args.length === 3) lastAlpha = true;
			args.push(value);
			argMeta.push(meta);
			return "";
		});
...
function getColor(color, options) {
	if (Array.isArray(color)) return color.map((c) => getColor(c, options));
	if (!color) throw new TypeError("Empty color reference");
	if (isString(color)) color = parse(color, options);
	let space = color.space || color.spaceId;
	if (typeof space === "string") color.space = ColorSpace.get(space);
...
var ColorSpace = class ColorSpace {
	...
	*/
	getFormat(format) {
		...
		if (format === "default") format = Object.values(this.formats)[0];
		else if (typeof format === "string") format = this.formats[format];
		let ret = Format.get(format, this);
		if (ret !== format && format.name in this.formats) this.formats[format.name] = ret;
		return ret;
	}
	/**
	...
	*/
	static get(space, ...alternatives) {
		if (!space || isInstance(space, this)) return space;
	...
	*/
	static findFormat(filters, spaces = ColorSpace.all) {
		...
		if (typeof filters === "string") filters = { name: filters };
		for (let space of spaces) for (let [name, format] of Object.entries(space.formats)) {
			format.name ??= name;
			format.type ??= "function";
			...
			if (matches) {
				...
				if (ret !== format) space.formats[format.name] = ret;
				return ret;
			}
		}
		return null;
...
function tryColor(color, options = {}) {
	if (Array.isArray(color)) return color.map((c) => tryColor(c, options));
	...
	if (isString(color) && element && CSS && getComputedStyle) {
		if (CSS.supports(cssProperty, color)) {
			...
			if (color !== previousValue) element.style[cssProperty] = color;
			let computedColor = getComputedStyle(element).getPropertyValue(cssProperty);
			if (color !== previousValue) element.style[cssProperty] = previousValue;
			if (computedColor !== color) try {
				return getColor(computedColor, getColorOptions);
			} catch (e) {
				error = e;
			}
			else error = { message: "Color value is a valid CSS color, but it could not be resolved :(" };
		}
	}
	if (options.errorMeta) options.errorMeta.error = error;
	return null;
}
...
function getAll(color, options) {
	...
	let coords;
	if (!space || color.space.equals(space)) coords = color.coords.slice();
	else coords = space.from(color);
...
function get(color, prop) {
	color = getColor(color);
	if (prop === "alpha") return color.alpha ?? 1;
...
function setAll(color, space, coords, alpha) {
	color = getColor(color);
	if (Array.isArray(space)) [space, coords, alpha] = [
		color.space,
		space,
		coords
	];
	...
	if (alpha !== void 0) color.alpha = alpha;
	return color;
}
/** @type {"color"} */
...
function set(color, prop, value) {
	...
	if (arguments.length === 2 && type(arguments[1]) === "object") {
		...
		if (prop === "alpha") color.alpha = value;
		else {
			let { space, index } = ColorSpace.resolveCoord(prop, color.space);
...
var lch = new ColorSpace({
	...
	fromBase(Lab) {
		...
		return [
			L,
			isAchromatic ? 0 : Math.sqrt(a ** 2 + b ** 2),
			h
		];
	},
	toBase(lch) {
...
function distance(color1, color2, space = "lab") {
	...
	return Math.sqrt(coords1.reduce((acc, c1, i) => {
		let c2 = coords2[i];
		if (isNone(c1) || isNone(c2)) return acc;
		return acc + (c2 - c1) ** 2;
	}, 0));
}
/**
...
var Jzazbz = new ColorSpace({
	...
	fromBase(XYZ) {
		let [Xa, Ya, Za] = XYZ;
		let [Iz, az, bz] = multiply_v3_m3x3(multiply_v3_m3x3([
			b$1 * Xa - (b$1 - 1) * Za,
			g * Ya - (g - 1) * Xa,
			Za
		], XYZtoCone_M).map(function(val) {
			return spow((c1$2 + c2$2 * spow(val / 1e4, n$1)) / (1 + c3$2 * spow(val / 1e4, n$1)), p);
		}), ConetoIab_M);
		return [
			(1 + d) * Iz / (1 + d * Iz) - d0,
			az,
	...
	toBase(Jzazbz) {
		let [Jz, az, bz] = Jzazbz;
		let [Xm, Ym, Za] = multiply_v3_m3x3(multiply_v3_m3x3([
			(Jz + d0) / (1 + d - d * (Jz + d0)),
			az,
			bz
		], IabtoCone_M).map(function(val) {
			return 1e4 * spow((c1$2 - spow(val, pinv)) / (c3$2 * spow(val, pinv) - c2$2), ninv$1);
		}), ConetoXYZ_M);
...
function deltaEJz(color, sample) {
	...
	if (isNone(Hz1) && isNone(Hz2)) {
		Hz1 = 0;
		Hz2 = 0;
	} else if (isNone(Hz1)) Hz1 = Hz2;
	else if (isNone(Hz2)) Hz2 = Hz1;
	let Δh = Hz1 - Hz2;
...
var ictcp = new ColorSpace({
	...
	base: XYZ_Abs_D65,
	fromBase(XYZ) {
		return LMStoICtCp(multiply_v3_m3x3(XYZ, XYZtoLMS_M));
	},
	toBase(ICtCp) {
		return multiply_v3_m3x3(ICtCptoLMS(ICtCp), LMStoXYZ_M);
	},
...
function LMStoICtCp(LMS) {
	return multiply_v3_m3x3(LMS.map(function(val) {
...
function ICtCptoLMS(ICtCp) {
	return multiply_v3_m3x3(ICtCp, IPTtoLMS_M).map(function(val) {
...
function fromCam16(cam16, env) {
	...
	return multiply_v3_m3x3(unadapt(multiply_v3_m3x3([
		p2,
		r * cosh,
		r * sinh
	], m1).map((c) => {
...
function toCam16(xyzd65, env) {
	const rgbA = adapt$1(multiply_v3_m3x3(xyzd65.map((c) => {
		return c * 100;
	}), cat16).map((c, i) => {
		return c * env.dRgb[i];
	}), env.fl);
...
var cam16 = new ColorSpace({
	...
	base: xyz_d65,
	fromBase(xyz) {
		if (this.ε === void 0) this.ε = Object.values(this.coords)[1].refRange[1] / 1e5;
...
* @returns {PlainColorObject}
*/
function toGamut(color, { method = defaults.gamut_mapping, space = void 0, deltaEMethod = "", jnd = 2, blackWhiteClamp = void 0 } = {}) {
	color = getColor(color);
...
function serialize(color, options = {}) {
	let { precision = defaults.precision, format, inGamut: inGamut$1 = true, coords: coordFormat, alpha: alphaFormat, commas } = options;
	let ret;
	...
	if (formatId) format = colorWithMeta.space.getFormat(format) ?? ColorSpace.findFormat(formatId);
	if (!format) {
		format = colorWithMeta.space.getFormat("default") ?? ColorSpace.DEFAULT_FORMAT;
		formatId = format.name;
	}
	if (format && format.space && format.space !== colorWithMeta.space) colorWithMeta = to(colorWithMeta, format.space);
	let coords = colorWithMeta.coords.slice();
	inGamut$1 ||= format.toGamut;
	if (inGamut$1 && !inGamut(colorWithMeta)) coords = toGamut(clone(colorWithMeta), inGamut$1 === true ? void 0 : inGamut$1).coords;
	if (format.type === "custom") if (format.serialize) ret = format.serialize(coords, colorWithMeta.alpha, options);
		...
		if (alphaFormat !== void 0 && !(typeof alphaFormat === "object")) alphaFormat = typeof alphaFormat === "string" ? { type: alphaFormat } : { include: alphaFormat };
...
var sRGB = new RGBColorSpace({
	...
	formats: {
		...
		hex: {
			...
			serialize: (coords, alpha, { collapse = true, alpha: alphaFormat } = {}) => {
				if (alphaFormat !== false && alpha < 1 || alphaFormat === true) coords.push(alpha);
...
function deltas(c1, c2, { space, hue = "shorter" } = {}) {
	...
	let coords = coords1.map((coord1, i) => {
		...
		if (coordMeta.type === "angle") [coord1, coord2] = adjust(hue, [coord1, coord2]);
...
function subtractCoords(c1, c2) {
	if (isNone(c1) || isNone(c2)) return c1 === c2 ? null : 0;
	return c1 - c2;
}
/** @import { ColorTypes } from "./types.js" */
/**
...
function range(color1, color2, options = {}) {
	...
	if (space.coords.h && space.coords.h.type === "angle") {
		...
		if (isNone(θ1) && !isNone(θ2)) θ1 = θ2;
		else if (isNone(θ2) && !isNone(θ1)) θ2 = θ1;
...
var prophoto = new RGBColorSpace({
	...
	fromBase(RGB) {
		return RGB.map((v) => {
			let sign = v < 0 ? -1 : 1;
			let abs = v * sign;
			if (abs >= Et) return sign * abs ** (1 / 1.8);
			return 16 * v;
		});
	}
});
...
var rec2020Oetf = new RGBColorSpace({
	...
	toBase(RGB) {
		return RGB.map(function(val) {
			let sign = val < 0 ? -1 : 1;
			let abs = val * sign;
			if (abs < β * 4.5) return val / 4.5;
			return sign * Math.pow((abs + α - 1) / α, 1 / .45);
		});
	},
	fromBase(RGB) {
		return RGB.map(function(val) {
			let sign = val < 0 ? -1 : 1;
			let abs = val * sign;
			if (abs >= β) return sign * (α * Math.pow(abs, .45) - (α - 1));
			return 4.5 * val;
		});
	}
});
...
function vdot(a, b) {
	let l = a.length;
	if (l !== b.length) throw new Error(`Vectors of size ${l} and ${b.length} are not aligned`);
	let s = 0;
	a.forEach((c, i) => {
		s += c * b[i];
	});
...
function findGamutIntersection(a, b, l1, c1, l0, lmsToRgb, okCoeff, cusp) {
	let t;
	if (cusp === void 0) cusp = findCusp(a, b, lmsToRgb, okCoeff);
	if ((l1 - l0) * cusp[1] - (cusp[0] - l0) * c1 <= 0) t = cusp[1] * l0 / (c1 * cusp[0] + cusp[1] * (l0 - l1));
		...
		let r_ = vdot(lmsToRgb[0], [
			l,
			m,
			s
		]) - 1;
		let r1 = vdot(lmsToRgb[0], [
			ldt,
			mdt,
			sdt
		]);
		let r2 = vdot(lmsToRgb[0], [
			ldt2,
			mdt2,
			sdt2
		]);
		let ur = r1 / (r1 * r1 - .5 * r_ * r2);
		let tr = -r_ * ur;
		let g_ = vdot(lmsToRgb[1], [
			l,
			m,
			s
		]) - 1;
		let g1 = vdot(lmsToRgb[1], [
			ldt,
			mdt,
			sdt
		]);
		let g2 = vdot(lmsToRgb[1], [
			ldt2,
			mdt2,
			sdt2
		]);
		let ug = g1 / (g1 * g1 - .5 * g_ * g2);
		let tg = -g_ * ug;
		let b_ = vdot(lmsToRgb[2], [
			l,
			m,
			s
		]) - 1;
		let b1 = vdot(lmsToRgb[2], [
			ldt,
			mdt,
			sdt
		]);
		let b2 = vdot(lmsToRgb[2], [
			ldt2,
			mdt2,
			sdt2
		]);
		let ub = b1 / (b1 * b1 - .5 * b_ * b2);
		let tb = -b_ * ub;
		tr = ur >= 0 ? tr : floatMax;
		tg = ug >= 0 ? tg : floatMax;
		tb = ub >= 0 ? tb : floatMax;
...
function getCs(lab, lmsToRgb, okCoeff) {
	...
	let ca = l * stMid[0];
	let cb = (1 - l) * stMid[1];
	let cMid = .9 * k * Math.sqrt(Math.sqrt(1 / (1 / ca ** 4 + 1 / cb ** 4)));
	ca = l * .4;
	cb = (1 - l) * .8;
	return [
...
function oklabToOkhsl(lab, lmsToRgb, okCoeff) {
	...
	let h = .5 + Math.atan2(-lab[2], -lab[1]) / tau;
	if (l !== 0 && l !== 1 && c !== 0) {
		let [c0, cMid, cMax] = getCs([
			L,
			lab[1] / c,
...
function okhsvToOklab(hsv, lmsToRgb, okCoeff) {
	...
	if (l !== 0 && s !== 0) {
		let a_ = Math.cos(tau * h);
		let b_ = Math.sin(tau * h);
		let [sMax, tMax] = toSt(findCusp(a_, b_, lmsToRgb, okCoeff));
		let s0 = .5;
		let k = 1 - s0 / sMax;
...
var rec2100Hlg = new RGBColorSpace({
	...
	fromBase(RGB) {
		return RGB.map(function(val) {
			val /= scale;
			if (val <= 1 / 12) return spow(3 * val, .5);
...
var Color = class Color {
	...
	*/
	constructor(...args) {
		...
		if (args.length === 1) {
			let parseMeta = {};
			if (typeof args[0] === "object" && Object.getPrototypeOf(args[0]).constructor === Object) args[0] = { ...args[0] };
			color = getColor(args[0], { parseMeta });
			if (parseMeta.format) this.parseMeta = parseMeta;
		}
		...
		this.alpha = isNone(alpha) ? alpha : alpha === void 0 ? 1 : clamp(0, alpha, 1);
	...
	*/
	static get(color, ...args) {
		if (isInstance(color, this)) return color;
