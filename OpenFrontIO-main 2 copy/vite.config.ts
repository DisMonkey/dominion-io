...
export default defineConfig(({ mode }) => {
  ...
  const apiDomain = env.VITE_API_DOMAIN ?? env.API_DOMAIN;
  const wsDomain = env.VITE_WS_DOMAIN ?? env.WEBSOCKET_URL;
  const htmlAssetData = {
    assetManifest: JSON.stringify(assetManifest),
    cdnBase: JSON.stringify(cdnBase),
    gameEnv: JSON.stringify(env.GAME_ENV ?? "dev"),
    manifestHref: buildAssetUrl("manifest.json", assetManifest, cdnBase),
    faviconHref: buildAssetUrl("favicon.svg", assetManifest, cdnBase),
    gameplayScreenshotUrl: buildAssetUrl(
      "images/DominionPreview.svg",
      assetManifest,
      cdnBase,
    ),
    ...
    desktopLogoImageUrl: buildAssetUrl(
      "images/DominionLogo.svg",
      assetManifest,
      cdnBase,
    ),
    mobileLogoImageUrl: buildAssetUrl("favicon.svg", assetManifest, cdnBase),
  };
  ...
  return {
    ...
    define: {
      ...
      "process.env.WEBSOCKET_URL": JSON.stringify(
        isProduction ? (wsDomain ?? "") : "localhost:3000",
      ),
