...
export class CameraShakeEvent implements GameEvent {
  constructor(
    public readonly intensity: number,
    public readonly durationMs: number = 360,
  ) {}
}
...
export class TransformHandler {
  ...
  constructor(
    ...
    this.eventBus.on(CameraShakeEvent, (e) => this.onCameraShake(e));
  }
  ...
  handleTransform(context: CanvasRenderingContext2D) {
    ...
    context.setTransform(
      ...
      0,
      this.scale,
      this.game.width() / 2 - this.offsetX * this.scale + shake.x,
      this.game.height() / 2 - this.offsetY * this.scale + shake.y,
    );
  }

  private onCameraShake(event: CameraShakeEvent): void {
    this.shakeIntensity = Math.min(
      18,
      Math.max(this.shakeIntensity, event.intensity),
    );
    this.shakeDurationMs = Math.max(1, event.durationMs);
    this.shakeRemainingMs = Math.max(this.shakeRemainingMs, event.durationMs);
    this.shakeLastUpdateMs = performance.now();
    this.shakeSeed = (this.shakeSeed + 1) % 1024;
  }

  private currentShakeOffset(): { x: number; y: number } {
    if (this.shakeRemainingMs <= 0 || this.shakeIntensity <= 0) {
      return { x: 0, y: 0 };
    }

    const now = performance.now();
    const deltaMs =
      this.shakeLastUpdateMs === 0 ? 16.667 : now - this.shakeLastUpdateMs;
    this.shakeLastUpdateMs = now;
    this.shakeRemainingMs = Math.max(0, this.shakeRemainingMs - deltaMs);

    const t = this.shakeRemainingMs / this.shakeDurationMs;
    const falloff = t * t;
    ...
    const phase = now * 0.055 + this.shakeSeed;
    return {
      x: Math.sin(phase * 1.73) * amp,
      y: Math.cos(phase * 2.17) * amp,
    };
  }
