...
import {
  decodeClientSocketMessage,
  sendServerMessage,
} from "./network/SocketPackets";
...
export class GameServer {
  ...
  private addListeners(client: Client) {
    ...
    client.ws.on("message", async (message) => {
      try {
        const decoded = decodeClientSocketMessage(message);
        if (!decoded.ok) {
        ...
        const clientMsg = decoded.message;
  ...
  public prestart() {
    ...
    this.activeClients.forEach((c) => {
      ...
      sendServerMessage(c.ws, prestartMsg.data);
    });
  }
  ...
  private handleSynchronization() {
    ...
    for (const c of outOfSyncClients) {
      ...
      sendServerMessage(c.ws, serverDesync.data);
    }
  }
