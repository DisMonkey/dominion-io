...
import { AiThreatAnalyzer } from "./AiThreatAnalyzer";
...
export class AiAttackBehavior {
  ...
  maybeAttack() {
    ...
    const threatAnalyzer = new AiThreatAnalyzer(this.game, this.player);
    const borderingPlayers = threatAnalyzer
      .rankAttackTargets([...borderingPlayerSet])
  ...
  // attackBestTarget receives borderingFriends and borderingEnemies sorted by strategic attack score.
  ...
  private getAttackStrategies(
    ...
    const afk = (): boolean => {
      // borderingEnemies is attack-score sorted, so first viable match is the best AFK target.
    ...
    const weakest = (): boolean => {
      if (borderingEnemies.length > 0) {
        // borderingEnemies is attack-score sorted, so first match is the best pressure target.
  ...
  private findTraitor(borderingEnemies: Player[]): Player | null {
    ...
    // borderingEnemies is attack-score sorted, so first match is the best traitor target.
    return (
  ...
  private findVictim(borderingEnemies: Player[]): Player | null {
    // borderingEnemies is attack-score sorted, so first match is the best opportunistic victim.
    return (
  ...
  private findVeryWeakEnemy(borderingEnemies: Player[]): Player | null {
    ...
    // borderingEnemies is attack-score sorted, so first match is the best very weak enemy.
