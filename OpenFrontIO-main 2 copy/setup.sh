...
echo "👤 Setting up dominion user..."
# Create dominion user if it doesn't exist
if id "dominion" &> /dev/null; then
    echo "User dominion already exists"
else
    useradd -m -s /bin/bash dominion
    echo "User dominion created"
fi

# Check if dominion is already in docker group
if groups dominion | grep -q '\bdocker\b'; then
    echo "User dominion is already in the docker group"
else
    # Add dominion to docker group
    usermod -aG docker dominion
    echo "Added dominion to docker group"
fi

# Create .ssh directory for dominion if it doesn't exist
if [ ! -d "/home/dominion/.ssh" ]; then
    mkdir -p /home/dominion/.ssh
    chmod 700 /home/dominion/.ssh
    echo "Created .ssh directory for dominion"
fi

# Copy SSH keys from root if they exist and haven't been copied yet
if [ -f /root/.ssh/authorized_keys ] && [ ! -f /home/dominion/.ssh/authorized_keys ]; then
    cp /root/.ssh/authorized_keys /home/dominion/.ssh/
    chmod 600 /home/dominion/.ssh/authorized_keys
    echo "SSH keys copied from root to dominion"
fi
...
# Set proper ownership for dominion's home directory
chown -R dominion:dominion /home/dominion
echo "Set proper ownership for dominion's home directory"
...
TRAEFIK_CONFIG_DIR="/home/dominion/traefik"
...
services:
  traefik:
    ...
    volumes:
      ...
      - /home/dominion/traefik/traefik.toml:/etc/traefik/traefik.toml:ro
      - /home/dominion/traefik/tls.toml:/etc/traefik/tls.toml:ro
      - /home/dominion/traefik/certs:/certs:ro
    networks:
      - web
EOF

# Give dominion ownership of config files but keep certs owned by root.
# Traefik runs as root inside its container so it can read them, but the
# dominion app user cannot access the TLS private key.
chown -R dominion:dominion "$TRAEFIK_CONFIG_DIR"
...
OTEL_CONFIG_DIR="/home/dominion/otel"
...
chown -R dominion:dominion "$OTEL_CONFIG_DIR"
...
echo "🎉 SETUP COMPLETE!"
echo "====================================================="
echo "The dominion user has been set up and has Docker permissions."
